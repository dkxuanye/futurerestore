...
void futurerestore::enterPwnRecovery(plist_t build_identity, std::string bootargs) {
    ...
    if (_client->image4supported) {
        ...
        if (_externalNonce) {
            info("External nonce mode enabled, using APTicket IM4M BNCH and skipping internal ApNonce hax.\n");
            _client->nonce_size = nonceelem.payloadSize();
            retassure(_client->nonce_size == 20 || _client->nonce_size == 32,
                      "External nonce has unexpected size: %d\n", _client->nonce_size);
            _client->nonce = (unsigned char *)malloc(_client->nonce_size);
            retassure(_client->nonce, "Failed to allocate external nonce buffer\n");
            memcpy(_client->nonce, nonceelem.payload(), _client->nonce_size);
            info("External nonce mode: skip internal ApNonce hax because caller already set com.apple.System.boot-nonce.\n");
            ...
            retassure(((recovery_client_new(_client) == IRECV_E_SUCCESS) ||
                       (mutex_unlock(&_client->device_event_mutex), 0)),
                      "Failed to connect to device in Recovery Mode after external nonce iBEC boot!");
